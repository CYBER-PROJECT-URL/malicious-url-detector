...
# ---------- Serial persistent build with Resume + Limit ----------

def build_enriched_dataset_serial_persistent(raw_data_path, max_limit=None):
    try:
    ...
    if os.path.exists(ENRICHED_CSV_FILE):
        ...
        if 'url_original' in df_cache.columns:
            processed_urls = set(df_cache['url_original'].astype(str))
            mask = ~df_raw['url_original'].astype(str).isin(processed_urls)
            df_to_process = df_raw[mask].copy()
        else:
            df_to_process = df_raw.copy()
        valid_domains_count = len(df_cache)
        print(f"✨ נמצאו {valid_domains_count} רשומות קיימות ב-CSV. נשארו לעיבוד: {len(df_to_process)}")
    else:
        df_cache = None
        df_to_process = df_raw.copy()
        valid_domains_count = 0
        print("✨ לא נמצא CSV קיים. מתחיל מאפס.")

    if df_to_process.empty:
        print("אין רשומות חדשות לעיבוד. מדלג ישר לשלב האימון.")
        if df_cache is None:
            return None
        return df_cache

    out_cols = None
    write_header = not os.path.exists(ENRICHED_CSV_FILE)

    for i, (_, row) in enumerate(df_to_process.iterrows()):
        if max_limit is not None and valid_domains_count >= max_limit:
            print(f"\n✅ הושג הגבול (סה\"כ {max_limit} דומיינים). עוצר חילוץ.")
            break
        ...
        if out_cols is None:
            out_cols = list(clean_feats.keys())

        df_row = pd.DataFrame([clean_feats], columns=out_cols)
        mode = 'a' if os.path.exists(ENRICHED_CSV_FILE) else 'w'
        df_row.to_csv(ENRICHED_CSV_FILE, mode=mode, index=False, header=write_header)
        write_header = False

        valid_domains_count += 1

        if (valid_domains_count % 50) == 0:
            print(f"נשמרו עד כה {valid_domains_count} דומיינים...")

        if API_DELAY_SECONDS > 0:
            time.sleep(API_DELAY_SECONDS)

    print(f"\n✅ סיום חילוץ סדרתי. סה\"כ נשמרו (כולל Cache): {valid_domains_count}")
...
if __name__ == "__main__":
    ...
    # דוגמה: ריצה עם Limit של 10000, ואח"כ אפשר להגדיל ל-50000 בריצה נוספת
    MAX_LIMIT = None  # או למשל 10000

    if os.path.exists(ENRICHED_CSV_FILE):
        df_final = pd.read_csv(ENRICHED_CSV_FILE)
        print(f"נטען קובץ קיים: {ENRICHED_CSV_FILE}, shape={df_final.shape}")
        if MAX_LIMIT is not None and len(df_final) < MAX_LIMIT:
            df_final = build_enriched_dataset_serial_persistent(RAW_DATA_PATH, max_limit=MAX_LIMIT)
    else:
        df_final = build_enriched_dataset_serial_persistent(RAW_DATA_PATH, max_limit=MAX_LIMIT)
